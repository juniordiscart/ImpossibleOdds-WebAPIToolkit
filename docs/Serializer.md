...
* `@var`: field-based annotation. This is used to define the type of data the field of your object will hold. The types written down should either simple (`int`, `string`, etc.), be available in the root namespace, or a fully qualified typename.
* `@deserialize-alias`: field-based annotation and only used during deserialization. By default, the deserializer will use the name of the field to check if the data is present. When an alias is defined, it will look for the alias instead. Multiple deserialization aliases can be defined for a single field.
* `@serialize-alias`: field-based annotation and only used during serialization. By default, the serializer will use the field's name. However, setting this annotation allows you to serialize it under a different name. Contrary to the `@deserialize-alias`, only one such annotation can be defined per field.
